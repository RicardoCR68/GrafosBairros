...
int TNode::getID() { return this->id; }

string TNode::getLabel() { return this->label; }
...
int TGrafo::getN() { return n; }

int TGrafo::getM() { return m; }

float **TGrafo::getKm() { return km; }

float **TGrafo::getMin() { return min; }

TNode *TGrafo::getLocals() { return locals; }
...
void TGrafo::insereN(int id, string label) {
  if (n == N) {
    cout << "CAPACIDADE MAXIMA ATINGIDA! NENHUMA MUDANÇA FEITA" << endl;
  }
...
int TGrafo::MinKey(int key[], bool set[]) {
...
void TGrafo::prim() {
  ...
  for (int count = 0; count < n - 1; ++count) {
    ...
    for (int v = 0; v < n; ++v) {
      if (km[u][v] && mstSet[v] == false && km[u][v] < key[v]) {
        parent[v] = u;
        key[v] = km[u][v];
      }
    }
